...
export function storeDeployJetton(src: DeployJetton) {
    return (builder: Builder) => {
        let b_0 = builder;
        b_0.storeUint(2023116568, 32);
        b_0.storeRef(src.jettonContent);
        b_0.storeCoins(src.initialSupply);
    };
}

export function loadDeployJetton(slice: Slice) {
    let sc_0 = slice;
    if (sc_0.loadUint(32) !== 2023116568) { throw Error('Invalid prefix'); }
    let _jettonContent = sc_0.loadRef();
    let _initialSupply = sc_0.loadCoins();
    return { $$type: 'DeployJetton' as const, jettonContent: _jettonContent, initialSupply: _initialSupply };
}
...
export type Core$Data = {
    $$type: 'Core$Data';
    owner: Address;
    totalChildContracts: bigint;
    tokenAddresses: Dictionary<bigint, Address>;
}

export function storeCore$Data(src: Core$Data) {
    return (builder: Builder) => {
        let b_0 = builder;
        b_0.storeAddress(src.owner);
        b_0.storeInt(src.totalChildContracts, 257);
        b_0.storeDict(src.tokenAddresses, Dictionary.Keys.BigInt(257), Dictionary.Values.Address());
    };
}

export function loadCore$Data(slice: Slice) {
    let sc_0 = slice;
    let _owner = sc_0.loadAddress();
    let _totalChildContracts = sc_0.loadIntBig(257);
    let _tokenAddresses = Dictionary.load(Dictionary.Keys.BigInt(257), Dictionary.Values.Address(), sc_0);
    return { $$type: 'Core$Data' as const, owner: _owner, totalChildContracts: _totalChildContracts, tokenAddresses: _tokenAddresses };
}

function loadTupleCore$Data(source: TupleReader) {
    let _owner = source.readAddress();
    let _totalChildContracts = source.readBigNumber();
    let _tokenAddresses = Dictionary.loadDirect(Dictionary.Keys.BigInt(257), Dictionary.Values.Address(), source.readCellOpt());
    return { $$type: 'Core$Data' as const, owner: _owner, totalChildContracts: _totalChildContracts, tokenAddresses: _tokenAddresses };
}

function loadGetterTupleCore$Data(source: TupleReader) {
    let _owner = source.readAddress();
    let _totalChildContracts = source.readBigNumber();
    let _tokenAddresses = Dictionary.loadDirect(Dictionary.Keys.BigInt(257), Dictionary.Values.Address(), source.readCellOpt());
    return { $$type: 'Core$Data' as const, owner: _owner, totalChildContracts: _totalChildContracts, tokenAddresses: _tokenAddresses };
...
function storeTupleCore$Data(source: Core$Data) {
    let builder = new TupleBuilder();
    builder.writeAddress(source.owner);
    builder.writeNumber(source.totalChildContracts);
    builder.writeCell(source.tokenAddresses.size > 0 ? beginCell().storeDictDirect(source.tokenAddresses, Dictionary.Keys.BigInt(257), Dictionary.Values.Address()).endCell() : null);
    return builder.build();
}

function dictValueParserCore$Data(): DictionaryValue<Core$Data> {
    return {
        serialize: (src, builder) => {
            builder.storeRef(beginCell().store(storeCore$Data(src)).endCell());
        },
        parse: (src) => {
            return loadCore$Data(src.loadRef().beginParse());
        }
    }
}
...
async function Jetton_init(owner: Address, content: Cell, initial_supply: bigint) {
    const __code = Cell.fromBase64('');
    const __system = Cell.fromBase64('');
...
const Jetton_errors: { [key: number]: { message: string } } = {
    ...
    30892: { message: `Only owner can deploy` },
...
const Jetton_types: ABIType[] = [
    ...
    {"name":"DeployJetton","header":2023116568,"fields":[{"name":"jettonContent","type":{"kind":"simple","type":"cell","optional":false}},{"name":"initialSupply","type":{"kind":"simple","type":"uint","optional":false,"format":"coins"}}]},
    {"name":"Core$Data","header":null,"fields":[{"name":"owner","type":{"kind":"simple","type":"address","optional":false}},{"name":"totalChildContracts","type":{"kind":"simple","type":"int","optional":false,"format":257}},{"name":"tokenAddresses","type":{"kind":"dict","key":"int","value":"address"}}]},
]
...
export class Jetton implements Contract {
    ...
    async send(provider: ContractProvider, via: Sender, args: { value: bigint, bounce?: boolean| null | undefined }, message: 'Mint:Close' | JettonMint | JettonBurnNotification | Deploy) {
